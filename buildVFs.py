...
def build_vf(args, slope=None):
    ...
    temp_otf = vf_output_name.with_suffix('.totf')
    ...
    feature_file = ROOT_DIR.joinpath('feature/features.fea')
    ...
    subprocess.call(
        ...
        ['buildmasterotfs', '--mkot', '-gs', '-d', designspace_file],
        stdout=STDOUT,
        stderr=STDERR
    )
    ...
    output_otf.rename(temp_otf)

    # Add shaping and other feature file information to font
    subprocess.call(
        # -k is for using 'post' table format 2
        ['makeotf', '-dr', '-r', '-V', '-omitMacNames', '-osv', '4', '-f', temp_otf, '-ff', feature_file, '-ds', designspace_file,'-o', output_otf],
        stdout=STDOUT,
        stderr=STDERR
    )
    ...
    if args.hinted:
        # hint the file
        subprocess.call(
            ['otfautohint', '--no-flex', output_otf],
            stdout=STDOUT,
            stderr=STDERR
        )
